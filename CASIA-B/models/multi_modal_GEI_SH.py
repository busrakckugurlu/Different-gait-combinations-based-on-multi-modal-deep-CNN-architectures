...
gallery_images_stck, gallery_infos_stck, probe_images_stck, probe_infos_stck = load_testing_data_stack("data/10_pieces_SH/test")
...
def extract_feature(images_list1, images_list2,net):
  ...
  for img1,img2 in zip(images_list1,images_list2):
    ...
    silh_x= np.expand_dims(img_one, axis=0)
...
for i in range(len(All_probe_angles_images_gei)):
    for j in range(len(All_gallery_angles_images_gei)):
        ...
        with g.as_default():
            ...
            silh_modal = load_model('weights/MobileNet_SH.ckpt')
            ...
            net = Model(inputs=[silh_modal.input, gei_modal.input], outputs=fused_output)
